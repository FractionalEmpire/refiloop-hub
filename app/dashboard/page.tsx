...
export default async function DashboardPage() {
  ...
  const [tasksRes, eodRes, commits, eligibleOwnersRes, pendingSkipTraceRes] = await Promise.all([
    ...
    getRecentCommits(5),
    // Eligible owners: skip traced + match, not DNC, not in terminal status
    supabase
      .from("owners")
      .select("id")
      .eq("skip_trace_match", true)
      .or("dnc.is.null,dnc.eq.false")
      .not("outreach_status", "in", '("dnc","contacted","call_later")'),
    // Owners still waiting on skip trace
    supabase
      .from("owners")
      .select("*", { count: "exact", head: true })
      .eq("skip_trace_eligible", true)
      .or("skip_trace_done.is.null,skip_trace_done.eq.false"),
  ]);
  ...
  // Count phones for eligible owners (Kirk's callable inventory)
  let eligiblePhones = 0;
  if (eligibleOwnerIds.length > 0) {
    const phonesRes = await supabase
      .from("owner_phones")
      .select("*", { count: "exact", head: true })
      .in("owner_id", eligibleOwnerIds);
    eligiblePhones = phonesRes.count || 0;
  }

  const DAILY_DIAL_TARGET = 250;
  const pipelineStatus =
    eligiblePhones < DAILY_DIAL_TARGET ? "critical" :
    eligiblePhones < DAILY_DIAL_TARGET * 2 ? "low" : "healthy";
    ...
    <AppShell user={user}>
      <div className="p-8 max-w-6xl">
        ...
        <div
          ...
          style={{
            background: pipelineStatus === "critical" ? "#2d1a1a" : pipelineStatus === "low" ? "#2d2200" : "#161b22",
            borderColor: pipelineStatus === "critical" ? "#f85149" : pipelineStatus === "low" ? "#d29922" : "#30363d",
          }}
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <span
              ...
              style={{
                background: pipelineStatus === "critical" ? "#f8514920" : pipelineStatus === "low" ? "#d2992220" : "#3fb95020",
                color: pipelineStatus === "critical" ? "#f85149" : pipelineStatus === "low" ? "#d29922" : "#3fb950",
              }}
            >
              {pipelineStatus === "critical"
                ? `BELOW DAILY TARGET — need ${DAILY_DIAL_TARGET - eligiblePhones} more phones`
                : pipelineStatus === "low"
                ? "Running low — less than 2 days of calls queued"
                : "Healthy"}
            </span>
          </div>
          <div className="grid grid-cols-3 gap-6">
            <div>
              ...
              >
                {eligiblePhones.toLocaleString()}
              </div>
              <div className="text-xs font-medium" style={{ color: "#c9d1d9" }}>Callable phones</div>
              <div className="text-xs mt-0.5" style={{ color: "#484f58" }}>
                Kirk&apos;s daily target: {DAILY_DIAL_TARGET.toLocaleString()}
              </div>
            </div>
            <div>
              <div className="text-3xl font-bold mb-1" style={{ color: "#58a6ff" }}>
                {eligibleOwnerCount.toLocaleString()}
              </div>
            ...
            <div>
              <div
                className="text-3xl font-bold mb-1"
                style={{ color: pendingSkipTrace > 500 ? "#d29922" : "#8b949e" }}
              >
                {pendingSkipTrace.toLocaleString()}
              </div>
